    ...
    while simulation_app.is_running() and episode_cnt < num_episodes:
        ...
        with torch.inference_mode():
            ...
            if "OpenAI" in args_cli.task and "Direct" in args_cli.task:  # use openai env, we need to check the noise, i.e., obs[state]'s obj_pos == obs[obs]'s obj_pos
                ...
                # cprint(f"obj_pos, obj_rot: {obj_pos}, {obj_rot}", 'yellow')

                # cprint(f"obj_pos_pred: {obj_pos_pred}", 'yellow')

                # cprint(f"Q, Q_pred: {Q}, {Q_pred}", 'yellow')

                action_state = state_check[..., -20:].squeeze(0)
                action_obs = obs_check[..., -20:]
                # cprint(f"action_state: {action_state}", 'yellow')
                # cprint(f"action_obs: {action_obs}", 'yellow')


                #    assertions to avoid discrepancies between two observations
                # cprint(f"obj_pos, obj_rot: {obj_pos}, {obj_rot}", 'yellow')
                # cprint(f"obj_pos_pred: {obj_pos_pred}", 'yellow')
                # cprint(f"Q, Q_pred: {Q}, {Q_pred}", 'yellow')
            ...
            if args_cli.camera_debug:
                ...
                for cam_id in range(camera_numbers):
                    # cprint(f"cam_id: {cam_id}", "cyan")
                    # cprint(f"axis: {axes}", "cyan")
                    # visualize the depth img
                    depth_example = obs[f"depth_img_0{cam_id}"][0]
                    rgba_example = obs[f"rgba_img_0{cam_id}"][0][..., :3]
                    # plt.imshow(depth_example.detach().cpu().numpy())
                    # plt.show()
                    # plt.imshow(rgba_example.detach().cpu().numpy())
                    # plt.show()
                    ax1 = axes[cam_id][0]
                    ax2 = axes[cam_id][1]
                    ...
                    # cprint(f"axes[cam_id]: {axes[cam_id]}", "cyan")
                plt.pause(1e-9)
            ...
            if args_cli.point_cloud_debug:
                ...
                # cprint(f"range of colors: {np.min(np.asarray(o3d_sampled.colors))} ~ {np.max(np.asarray(o3d_sampled.colors))}", "cyan")
                # cprint(f"points.shape: {np.asarray(o3d_sampled.points).shape}", "cyan")
                # cprint(f"colors.shape: {np.asarray(o3d_sampled.colors).shape}", "cyan")
                # visualize point cloud 
            ...
            if success:
                ...
                if len(action_list_sub) < 20 or len(action_list_sub) > 150:
