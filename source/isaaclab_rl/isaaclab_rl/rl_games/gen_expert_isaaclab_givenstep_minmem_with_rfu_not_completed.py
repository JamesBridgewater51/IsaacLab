...
assert(args_cli.task in [
     ...
     "Isaac-Repose-Cube-Shadow-OpenAI-FF-Direct-HandInit-PointCloud-Tactile-SingleCam-GivenStep-v0",
...
def main():
    ...
    # rfu env initialization
    obj_name = args_cli.task_name
    env_rfu = RFUniverseBaseEnv(check_version=False)
    env_rfu.SetViewTransform(position=[0.0, 0.9, -0.15])
    env_rfu.ViewLookAt([0, 0.5, -0.5])
    env_rfu.SendObject("LoadMesh", os.path.join(obj_root_dir, f"{obj_name}/fbx.fbx"))
    env_rfu.step()

    object_rfu = env_rfu.GetAttr(654822)
    object_rfu.SetKinematic(True)
    object_rfu.SetScale(scale_dict[obj_name])
    shadow = env_rfu.GetAttr(456743)
    joint_names = shadow.data["names"]
    csc_rfu = csc(["right", "up", "forward"], ["right", "forward", "up"])
    ...
    for key in tactile_pos_names:
        ...
        for tmp_lst in dis_pos_list:
            ...
            local_position_list = csc_rfu.cs1_pos_to_cs2_pos(tmp_lst[2:])
            local_position_list_copy = copy.deepcopy(local_position_list)
            joint2point_pos_sub.append(local_position_list)

        joint2dis_indices.append(torch.tensor(joint2dis_indices_sub, dtype=torch.int64, device=rl_device))
        joint2point_pos.append(torch.tensor(joint2point_pos_sub, dtype=torch.float32, device=rl_device))
    ...
    while simulation_app.is_running() and episode_cnt < num_episodes:
        ...
        if episode_cnt % args_cli.store_every == 0 and episode_success and (not episode_failed) and (not episode_timeout):  
            ...
            if args_cli.record_tactile:
                ...
                cprint(f"tactile_chunk_size: {tactile_chunk_size}", "cyan")
