...
Modify to record the whole whole episode data, as long as the cube does not fall
...
assert(args_cli.task in [
     "Isaac-Repose-Cube-Shadow-Direct-Face-Down-Reorient-PC-Tactile-Multi-Target-v0",
     ...
     "Isaac-Repose-Cube-Shadow-OpenAI-FF-Direct-HandInit-PointCloud-Tactile-SingleCam-FixWrist-MultiTarget-v0",
     ])
...
def main():
    ...
    ######## !!!!!! ADD TO RECORD THE WHOLE CONSECUTIVE SUCCESS TIMES #################
    ...
    ################# ADD EPISODE LIST SUB ###############################
    episode_ends_list_sub = []
    #################################################################
    if args_cli.record_tactile:
        tactile_list_sub = []
    ...
    while simulation_app.is_running() and episode_cnt < num_episodes:
        ...
        with torch.inference_mode():
            ...
            while len(action_list_sub) < args_cli.max_episode_steps:
                ...
                if success:  # do not record data here
                    ...
                    ###### ADD TO COLLECT THE "EPISODE ENDS SUB" ######
                    steps_now = global_steps + len(pointcloud_list_sub)
                    episode_ends_list_sub.append(steps_now)
                    cprint(f"success in local step {len(pointcloud_list_sub)}, global step {steps_now}", "green")
                    last_success_local_step = len(pointcloud_list_sub)
                    #################################################
                ...
                if len(pointcloud_list_sub) == args_cli.max_episode_steps:  # timeout to check the success time and record
                    ...
                    if episode_success_steps > args_cli.episode_success_threshold:
                        ...
                        cprint(f"step recorded in episode {episode_cnt}, reward: {reward}, success_times: {episode_success_steps}", "green")
                        cprint(f"Episode steps: {len(action_list_sub)}", "green")
                        cprint(f"success timestamps: {episode_ends_list_sub}", "green")

                        episode_cnt += 1
                        ###### !!!!!!!  ADD TO RECORD THE TOTAL SUCCESS TIMES#######
                        total_success_time += len(episode_ends_list_sub)
                        ############################################################
                        ...
                        if args_cli.step_every is not None:
                            ...
                            #### ADD EPISODE END SUB ####
                            episode_ends_list_sub = fullfill_list(episode_ends_list_sub, 1, args_cli.step_every)
                            ############################
                            if args_cli.record_tactile:
                        ...
                        # record the episode
                        if args_cli.record_tail_steps == 0:
                            # record_steps = episode_ends_list_sub[-1]  # record til the last success step

                            # global_steps += record_steps
                            global_steps += last_success_local_step
                            episode_ends_list.extend(copy.deepcopy(episode_ends_list_sub))
                            pointcloud_list.extend(copy.deepcopy(pointcloud_list_sub[:last_success_local_step]))
                            state_list.extend(copy.deepcopy(state_list_sub[:last_success_local_step]))
                            obs_list.extend(copy.deepcopy(obs_list_sub[:last_success_local_step]))
                            action_list.extend(copy.deepcopy(action_list_sub[:last_success_local_step]))
                            agent_pos_list.extend(copy.deepcopy(agent_pos_list_sub[:last_success_local_step]))
                            if args_cli.record_tactile:
                                cprint(f"recording tactile_list_sub, len(sub): {last_success_local_step}", "cyan")
                                tactile_list.extend(copy.deepcopy(tactile_list_sub[:last_success_local_step]))
                                cprint(f"recording tactile_list, len(list): {len(tactile_list)}", "cyan")
                                                          
                        else :
                            raise ValueError("record_tail_steps must be 0 in multitarget situation")

                        # reset lists
                        episode_ends_list_sub.clear()
                        pointcloud_list_sub.clear()
                        state_list_sub.clear()
                        obs_list_sub.clear()
                        action_list_sub.clear()
        ...
        if episode_cnt % args_cli.store_every == 0 and episode_success and (not episode_failed) and (not episode_timeout):  
            ...
            if args_cli.record_tactile:
                ...
                if len(tactile_arrays.shape) == 3:                
                    tactile_chunk_size = (2000, tactile_arrays.shape[1], tactile_arrays.shape[2])
                elif len(tactile_arrays.shape) == 2:
                    tactile_chunk_size = (2000, tactile_arrays.shape[1])
                else: 
                    raise NotImplementedError("tactile data shape not supported")
