...
from isaaclab.sensors import CameraCfg, TiledCameraCfg, Camera, TiledCamera, ContactSensorCfg, ContactSensor
...
# # 加载 USD 文件
# stage = Usd.Stage.Open('path_to_your_file.usd')
# rootPrim = stage.GetPseudoRoot()

# # 遍历 USD 文件中的所有几何体
# for prim in rootPrim.Traverse():
#     if isinstance(prim, UsdGeom.Mesh):
#         mesh = pymesh.Mesh()
#         # 提取顶点
#         mesh.vertices = prim.GetVerticesAttr().Get()
#         # 提取面
#         mesh.faces = prim.GetFaceVertexIndicesAttr().Get()
#         # 保存为 .mesh 文件
#         pymesh.save_mesh("output_mesh.mesh", mesh, ascii=True)

def load_usd_as_mesh(usdfilename):
    stage = Usd.Stage.Open(usdfilename)
    mesh = UsdGeom.Mesh(stage.GetPrimAtPath('/path/to/mesh'))
    rootPrim = stage.GetPseudoRoot()
...
class InHandManipulationHandInitPCTactileEnv(InHandManipulationEnv):
    ...
    def __init__(self, cfg: AllegroHandEnvCfg | ShadowHandEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        offset=TiledCameraCfg.OffsetCfg(pos=(0.6, 0.6, 1.3), rot=(0.29, 0.147, 0.429, 0.843), convention="opengl"),
        ...
        offset=TiledCameraCfg.OffsetCfg(pos=(-0.8, 0.3, 1.3), rot=(-0.314, -0.151, 0.419, 0.838), convention="opengl"),
        # offset=CameraCfg.OffsetCfg(pos=(0.2, 0.05, 0.58), rot=(0.25, 0.19, 0.58, 0.75), convention="opengl"),
    )
    ...
    # )
        ...
        # add contact sensors: 
        self.contact_forces_cfg = ContactSensorCfg(
        prim_path="/World/envs/env_.*/Robot/robot0_(?!hand_mount|forearm).*", history_length=2, debug_vis=True
    )    
        # hand_mount and forearm are not related to the manipulating process
    
    #     self.contact_forces_cfg = ContactSensorCfg(
    #     prim_path="/World/envs/env_.*/Robot/robot0_.*", history_length=2, debug_vis=True
    # )
        ...
        # cprint(f"cfg: {cfg.robot_cfg.spawn}", "magenta")
    ...
    # )
        ...
        self.goal_rot_arr = torch.tensor([[ 0.0088, -0.0053,  0.8549, -0.5187],], device=self.device, dtype=torch.float)
        # [ 0.0643, -0.0972, -0.5482,  0.8282],
        # [ 0.5142, -0.8564,  0.0245, -0.0407],
        # [ 0.7903,  0.4122,  0.4020,  0.2097],
        # [ 0.3518, -0.3129,  0.6592, -0.5863],
        # [ 0.0370, -0.7293, -0.0346,  0.6824],
        # [ 0.5660,  0.7804, -0.1560, -0.2150],
        # [ 0.2865, -0.4841,  0.4211, -0.7115],
        # [ 0.5378,  0.6767, -0.3128, -0.3936],
        # [ 0.4518,  0.7662, -0.2321, -0.3937],
        # [ 0.0319, -0.2184, -0.1408,  0.9651],
        # [ 0.6213,  0.0690, -0.7758, -0.0862],
        # [ 0.2598,  0.0216,  0.9621,  0.0801],
        # [ 0.1071, -0.9689, -0.0245,  0.2216],
        # [ 0.0224, -0.9784, -0.0047,  0.2054],
        # [ 0.4022,  0.7474,  0.2506,  0.4657],
        # [ 0.2213, -0.9498, -0.0502,  0.2155],
        # [ 0.0442, -0.7603,  0.0376, -0.6469],
        # [ 0.2230, -0.3072,  0.5435, -0.7487],
        # [ 0.0330,  0.0658,  0.4474,  0.8913]])
    ...
    def _setup_scene(self):
        ...
        self.camera_01 = TiledCamera(self.camera_config_01)
        self.contact_forces = ContactSensor(self.contact_forces_cfg)
    ...
    def _get_observations(self) -> dict:
        ...
        # set contact_data to 0 if the contact_data is too small (< 0.1), end to 1 if data > 0.1
        # contact_data = torch.where(contact_data < 0.1, torch.zeros_like(contact_data), torch.ones_like(contact_data))


        # cprint(f"contact_data: {contact_data}", "cyan")
        # cprint(f"contact_data.shape: {contact_data.shape}", "cyan")
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # lower the object
        if self.cfg.object_name == "cube":
            object_default_state[:, 2] -= 0.06

        else:
            object_default_state[:, 2] -= 0.04
