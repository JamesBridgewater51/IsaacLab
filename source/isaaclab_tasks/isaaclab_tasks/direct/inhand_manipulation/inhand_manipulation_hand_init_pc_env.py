...
from isaaclab.sensors import CameraCfg, TiledCameraCfg, Camera, TiledCamera
...
class InHandManipulationHandInitPCEnv(InHandManipulationEnv):
    ...
    # )
        ...
        cfg.episode_length_s = 1000000.0
        cfg.max_consecutive_success = 1000.0  # impossible to reach
    ...
    # )
        ...
        # reset_env_ids = torch.tensor(range(self.num_envs), dtype=torch.float, device=self.device)

        # rand_floats = sample_uniform(-1.0, 1.0, (len(reset_env_ids), 2), device=self.device)
        # self.goal_rot_arr = randomize_rotation(
        #     rand_floats[:, 0], rand_floats[:, 1], self.x_unit_tensor[reset_env_ids], self.y_unit_tensor[reset_env_ids]
        # )

        self.goal_rot_arr = torch.tensor([[0.2865,-0.4841,0.4211,-0.7115],], device=self.device, dtype=torch.float)
    ...
    def _get_rewards(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if len(goal_env_ids) > 0:
            # one_line_difference
            self._reset_idx(goal_env_ids)   # reset the object to the initial state and randomize the goal 
