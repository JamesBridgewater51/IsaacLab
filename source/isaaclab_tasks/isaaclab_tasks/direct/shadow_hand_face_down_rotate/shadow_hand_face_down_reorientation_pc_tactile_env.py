...
from isaaclab_tasks.direct.shadow_hand_face_down_rotate import ShadowHandFaceDownRotateLiftEnv
from isaaclab_tasks.direct.shadow_hand_face_down_rotate import ShadowHandDirectFaceDownReorientPCTactileEnvCfg
from isaaclab_tasks.direct.shadow_hand_face_down_rotate.shadow_hand_face_down_rotate_lift_env_cfg import ShadowHandDirectFaceDownReorientPCTactileMultiObjectEnvCfg

from isaaclab_tasks.direct.shadow_hand_face_down_rotate.shadow_hand_face_down_rotate_lift_env import randomize_rotation, compute_rewards
...
class ShadowHandDirectFaceDownReorientPCTactileSingleCamEnv(ShadowHandFaceDownRotateLiftEnv):
    cfg: ShadowHandDirectFaceDownReorientPCTactileEnvCfg | ShadowHandDirectFaceDownReorientPCTactileMultiObjectEnvCfg
    
    def __init__(self, cfg: ShadowHandDirectFaceDownReorientPCTactileEnvCfg | ShadowHandDirectFaceDownReorientPCTactileMultiObjectEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # overwrite self.goal_pos
        '''The sequence cannot be changed since MULTI-CFG is a child class of SINGLE-CFG'''
        # cprint(f"ismulti: {isinstance(cfg, ShadowHandDirectFaceDownReorientPCTactileMultiObjectEnvCfg)}", "red")
        # cprint(f"ismulti: {isinstance(cfg, ShadowHandDirectFaceDownReorientPCTactileEnvCfg)}", "cyan")
        if isinstance(cfg, ShadowHandDirectFaceDownReorientPCTactileMultiObjectEnvCfg):
            self.goal_pos[:, :] = torch.tensor([0.1, -0.25, 0.5], device=self.device)
        elif isinstance(cfg, ShadowHandDirectFaceDownReorientPCTactileEnvCfg):
            self.goal_pos[:, :] = torch.tensor([-0.10, -0.52, 0.65], device=self.device)

        else: 
            raise ValueError("cfg should be either ShadowHandDirectFaceDownReorientPCTactileEnvCfg or ShadowHandDirectFaceDownReorientPCTactileMultiObjectEnvCfg")
        ...
        # cprint(f"self.hand.body_names: {self.hand.body_names}", "cyan")
        # cprint(f"self.contact_forces.body_names: {self.contact_forces.body_names}", "cyan")
    ...
    def _setup_scene(self):
        ...
        # cprint(f"self.hand.joint_names: {self.hand.joint_names}", "cyan")
    ...
    def _get_observations(self) -> dict:
        ...
        if self.cfg.observation_noise_model:
            obs_origin["agent_pos"] = self._observation_noise_model(obs_origin["agent_pos"])
