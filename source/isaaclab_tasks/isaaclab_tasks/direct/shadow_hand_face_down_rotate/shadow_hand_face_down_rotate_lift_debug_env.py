...
from isaaclab_tasks.direct.shadow_hand_face_down_rotate.shadow_hand_face_down_rotate_lift_env_cfg import ShadowHandDirectFaceDownRotateLiftEnvDebugCfg

from termcolor import cprint


class ShadowHandFaceDownRotateLiftDebugEnv(DirectRLEnv):
    cfg: ShadowHandDirectFaceDownRotateLiftEnvDebugCfg

    def __init__(self, cfg: ShadowHandDirectFaceDownRotateLiftEnvDebugCfg, render_mode: str | None = None, **kwargs):
        ...
        #### test in 10.27 ###########
        self.goal_pos[:, :] = torch.tensor([-0.15, -0.45, 0.68], device=self.device)
    ...
    def _setup_scene(self):
        ...
        # add fingertip sensors
        if self.cfg.fingertip_contact_sensor_cfg is not None:
            self.fingertip_force = ContactSensor(self.cfg.fingertip_contact_sensor_cfg)
        else:
            self.fingertip_force = None
        ...
        if self.fingertip_force is not None:
            self.scene.sensors["fingertip_force_sensors"] = self.fingertip_force
        # add lights
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if self.fingertip_force is not None:
            fingertip_contact_force = self.fingertip_force.data.net_forces_w
            # shape: [num_envs, num_contacts, 3], 3 means the xyz forces
            binary_finger_contact = torch.norm(fingertip_contact_force, p=2, dim=-1) > 0.0
            # shape: [num_envs, num_contacts], 1 means contact, 0 means no contact
            num_contact = torch.sum(binary_finger_contact, dim=-1)
        else:
            num_contact = None
        ...
        (
            ...
            self.cfg.num_fingers,
            num_contact,
            self.cfg.fingertip_contact_reward_scale,
...
def compute_rewards(
    ...
    num_fingers: float,
    num_contact: torch.Tensor | None,
    fingertip_contact_reward_scale: float,
    ...
    if fingertip_contact_reward_scale < 0 and num_contact is not None:
        num_not_contact = num_fingers - num_contact
        fingertip_contact_reward = num_not_contact.float() * fingertip_contact_reward_scale
        reward_terms['fingertip_contact_reward'] = fingertip_contact_reward
    
    ################ end of computing additional rewards ####################
    # cprint(f"reward_terms: {reward_terms}", "red")
    # cprint(f"num_contact: {num_contact}", "red")
    # cprint(f"num_not_contact: {num_not_contact}", "red")
    ...
    # cprint(f"dist_rew: {dist_rew}", "red")
    # cprint(f"rot_rew: {rot_rew}", "red")
    # cprint(f"action_penalty* action_penalty_scale: {action_penalty* action_penalty_scale}", "red")
