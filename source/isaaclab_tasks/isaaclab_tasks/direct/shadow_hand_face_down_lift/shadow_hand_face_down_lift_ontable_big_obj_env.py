...
from isaaclab_tasks.direct.shadow_hand_face_down_lift.shadow_hand_face_down_lift_env_big_obj_cfg import ShadowHandDirectDownLiftOntableBigObjEnvCfg

from termcolor import cprint


class ShadowHandFaceDownLiftOntableBigObjectEnv(DirectRLEnv):
    cfg: ShadowHandDirectDownLiftOntableBigObjEnvCfg

    def __init__(self, cfg: ShadowHandDirectDownLiftOntableBigObjEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.expected_pos = self.in_hand_pos[:].clone()
        self.expected_pos[..., 2] = self.cfg.expected_height
        cprint(f"self.in_hand_pos: {self.in_hand_pos}", "cyan") # [ 0.0000, -0.3750,  0.0000]
        cprint(f"self.expected_pos: {self.expected_pos}", "cyan")
        cprint(f"self.success_ftip_height: {self.cfg.success_ftip_height}", "cyan")
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        (
            ...
            self.cfg.goal_reach_tolerance,
            self.cfg.success_ftip_height,
            self.cfg.stage,
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        goal_dist = torch.norm(self.object_pos - self.expected_pos, p=2, dim=-1)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # MMMMMODIFIED; ONLY APPLY NOISE TO X AND Y AXIS
        pos_noise = sample_uniform(-1.0, 1.0, (len(env_ids), 2), device=self.device)
        object_default_state[:, 0:2] = (
            object_default_state[:, 0:2] + self.cfg.reset_position_noise * pos_noise + self.scene.env_origins[env_ids][:, 0:2]
        )

        object_default_state[:, 2] = object_default_state[:, 2] + self.scene.env_origins[env_ids][:, 2]
...
def compute_rewards(
    ...
    hit_threshold: float , 
    hit_penalty: float ,
    # expected_pos:  float,    # given by "target_pos"
    goal_reach_tolerance: float,
    success_ftip_height: float,
    stage_number: int,
    ...
    # computing reward terms inside a dict
    reward_terms = dict()
    ...
    if stage_number == 1:
        ...
        reward_terms["goal_dist_rew"] = dist_rew
        ...
        reward_terms["goal_dist_rew"] = dist_rew
        reward_terms["goal_rot_rew"] = rot_rew

    else:
        raise NotImplementedError(f"stage number {stage_number} not supported")


    ################## reward that are computed anyway ####################
    #######################################################################
    action_penalty = torch.sum(actions**2, dim=-1)    
    reward_terms["action_penalty"] = action_penalty * action_penalty_scale  # scale < 0
    ...
    ################ end of computing rewards ####################
    # cprint(f"reward_terms: {reward_terms}", "red")
    # cprint(f"goal_dist: {goal_dist}", "red")
    # cprint(f"contact_force: {table_contact_force}", "cyan")
    # cprint(f"object_pos: {object_pos}", "cyan")
    ...
    # cprint(f"fingertip_pos_z >= success_ftip_height: {fingertip_pos_z >= success_ftip_height}", "red")
    # cprint(f"torch.all(fingertip_pos_z >= success_ftip_height, dim=-1): {torch.all(fingertip_pos_z >= success_ftip_height, dim=-1)}", "red")
    # cprint(f"self.fingertip_pos_z: {fingertip_pos_z}", "cyan")  #  tensor([[0.5216, 0.5137, 0.5125, 0.5162, 0.5094]], device='cuda:0')
    if stage_number == 1:
        goal_resets = torch.where(
            torch.abs(goal_dist) <= goal_reach_tolerance, \
            torch.ones_like(reset_goal_buf) & (object_linvel_norm <= obj_lin_vel_thresh) & (object_angvel_norm <= obj_ang_vel_thresh) & (dof_vel_norm <= dof_vel_thresh) & (torch.all(fingertip_pos_z >= success_ftip_height, dim=-1)), \
            reset_goal_buf
            )
    elif stage_number == 2:
        goal_resets = torch.where(
            torch.abs(goal_dist) <= goal_reach_tolerance, \
            torch.ones_like(reset_goal_buf) & (object_linvel_norm <= obj_lin_vel_thresh) & (object_angvel_norm <= obj_ang_vel_thresh) & (dof_vel_norm <= dof_vel_thresh) & (torch.all(fingertip_pos_z >= success_ftip_height, dim=-1)) & (torch.abs(rot_dist) <= success_tolerance), \
            reset_goal_buf
            )
    else:
        raise NotImplementedError(f"stage: {stage_number} not implemented")

    if penalize_table_contact:
        goal_resets = goal_resets & (torch.abs(table_contact_force).sum(-1).view(num_envs,) == 0.0) & (object_pos[..., 2] > object_hit_thresh)
    

    # horizontal_dist = torch.norm(object_pos, goal)x
