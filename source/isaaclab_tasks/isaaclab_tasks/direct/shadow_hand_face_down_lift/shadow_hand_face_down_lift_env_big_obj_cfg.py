...
class ShadowHandEnvCfg(DirectRLEnvCfg):
    ...
    reset_position_noise = 0.05  # range of position at reset
...
@configclass
class ShadowHandDirectDownLiftOntableBigObjEnvCfg(ShadowHandEnvCfg):
    ...
    if stage == 1: # train lifting, we dont have the rot_reward so 不需要拦住它主动重置
        ...
        obj_pos = (0.0, -0.43, table_top_pos + 0.035)
        dist_reward_scale = -4.0


        
    object_table_pos = table_top_pos + _object_scale[2] * 0.03 # 边长 / 2    
    # success_ftip_height = (hand_bottom_pos - table_top_pos) * 1 / 3 + object_table_pos  
    success_ftip_height = 0.0  # no liminations as long as not hie
    table_contact_force_scale = 1.0
    object_hit_thresh = 0.03
    expected_height = (hand_bottom_pos - table_top_pos) * 2 / 3 + object_table_pos  # 0.5402
