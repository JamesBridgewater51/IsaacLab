...
from isaaclab_tasks.direct.shadow_hand_face_down_lift.shadow_hand_face_down_lift_env_cfg import ShadowHandOpenAIFaceDownLiftEnvCfg, ShadowHandEnvCfg

from termcolor import cprint


class ShadowHandFaceDownLiftEnv(DirectRLEnv):
    cfg: ShadowHandOpenAIFaceDownLiftEnvCfg

    def __init__(self, cfg: ShadowHandOpenAIFaceDownLiftEnvCfg, render_mode: str | None = None, **kwargs):

        cfg.viewer.eye = (1.0, -0.3, 0.2)
        cfg.viewer.lookat = (0.0, -0.4, 0.0)
        ...
        self.in_hand_pos[:, 2] = self.hand.data.default_root_state[:, 2] / 2  # set z axis half
        ...
        self.goal_pos[:, :] = torch.tensor([-0.2, -0.45, 0.25], device=self.device)
    ...
    def _setup_scene(self):
        ...
        # add tables
        # self.table = RigidObject(self.cfg.table_rigid_cfg)
        ...
        # add sensors
        # self.contact_sensors_table = ContactSensor(self.cfg.contact_forces_table_cfg)
    ...
    def _get_observations(self) -> dict:
        ...
        # cprint(f"self.scene.sensors['contact_sensors_table'].force_matrix_w: {contact_sensors.data.net_forces_w}", "light_yellow")
    ...
    def _get_rewards(self) -> torch.Tensor:
        (
            ...
            # self.cfg.clip_energy_reward,
            # self.cfg.energy_upper_bound
...
def compute_rewards(
    ...
    相比原来的代码改动的地方：
    1. goal_reach条件改变
    # 2. 取消action_penalty，改为energy_cost  # 这一项后面又改回来了
    3. 增加contact detect(这个是自己又改过的，和哪个代码都不一样)
    4. 增加fingertip close reward
    5. 暂时不用原环境中对action_norm对goal_reach的限制，因为不太知道shadow hand的合理的action norm是多少
    6. 取消了之前的dist_reward(原来的这个dist reward略显有点怪)
    '''
    ...
    # 暂时放弃energy penalty, 还是用action penalty 代替
    action_penalty = torch.sum(actions**2, dim=-1)
    reward_terms["action_penalty"] = action_penalty * action_penalty_scale  # action_penalty_scale < 0


    if penalize_tb_contact:
        # in_contact = torch.abs(table_cf).sum(-1) > 0.2
        in_contact = object_pos[..., 2] < 0.05
        reward_terms['tb_contact_reward'] = -in_contact.float() * tb_cf_scale

    dof_vel_norm = torch.linalg.norm(dof_vel, dim=-1)

    goal_reach = (abs_rot_dist <= success_tolerance) & (dof_vel_norm <= dof_vel_thresh) & (object_linvel_norm <= obj_lin_vel_thresh) & (object_angvel_norm <= obj_ang_vel_thresh)

    if penalize_tb_contact:
        goal_reach = goal_reach & (object_pos[..., 2] < 0.05)
    ...
    # cprint(f"object_pos: {object_pos}", "green")
    # cprint(f"reward_terms: {reward_terms}", "cyan")
    '''
    object_pos: tensor([[-0.0288, -0.2621,  0.0300],
        [ 0.0289, -0.3913,  0.0300]], device='cuda:0')
    reward_terms: {'ftip_reward': tensor([-0.1807, -0.0711], device='cuda:0'), 'rot_reward': tensor([0.4580, 0.4166], device='cuda:0'), 'action_penalty': tensor([-0.0021, -0.0021], device='cuda:0'), 'tb_contact_reward': tensor([-1., -1.], device='cuda:0')}
    '''
...
#     # obj_pos_xy: torch.Tensor,
#     # inhand_pos_xy: torch.Tensor,
# ):
#     goal_dist = torch.norm(object_pos - target_pos, p=2, dim=-1)
#     # goal_dist = torch.norm(self.object_pos[..., :2] - self.in_hand_pos[..., :2], p=2, dim=-1)
#     # goal_dist = torch.norm(obj_pos_xy - inhand_pos_xy, p=2, dim=-1)
#     rot_dist = rotation_distance(object_rot, target_rot)
