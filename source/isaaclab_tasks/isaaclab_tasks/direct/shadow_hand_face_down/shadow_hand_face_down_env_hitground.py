...
from isaaclab_tasks.direct.shadow_hand_face_down.shadow_hand_face_down_env_cfg import  ShadowHandOpenAIFaceDownHitGroundEnvCfg
...
class ShadowHandFaceDownHitGroundEnv(DirectRLEnv):
    cfg: ShadowHandOpenAIFaceDownHitGroundEnvCfg 

    def __init__(self, cfg: ShadowHandOpenAIFaceDownHitGroundEnvCfg, render_mode: str | None = None, **kwargs):

        cfg.viewer.eye = (1.0, -0.3, 0.6)
        # cfg.viewer.eye = (-0.15, -0.8, 0.1)
        cfg.viewer.lookat = (0.0, -0.4, 0.1)
        ...
        self.goal_pos[:, :] = torch.tensor([-0.2, -0.45, 0.2], device=self.device)
    ...
    #############################   re-definition of functions from DirectRLEnv ################################
    ...
    def _get_observations(self) -> dict:
        ...
        # cprint(f"self.fingertip_pos.shape: {self.fingertip_pos.shape}", "light_yellow")  # self.fingertip_pos.shape: torch.Size([2, 5, 3])
        # cprint(f"self.fingertip_pos: {self.fingertip_pos}", "light_yellow")
        # cprint(f"min_tensor: {torch.min(self.fingertip_pos[..., 2])}", "cyan")

        # if torch.any(torch.isclose(self.fingertip_pos[..., 2], torch.tensor(0.0, device=self.device), atol=0.04)):
            # cprint(f"self.fingertip_pos: {self.fingertip_pos}", "light_yellow")
            # time.sleep(5)
    ...
    def _get_rewards(self) -> torch.Tensor:
        (
            ...
            self.cfg.hit_threshold,
            self.cfg.hit_penalty,
        )

        # cprint(f"num: {self.fingertip_pos[..., 2]}", "red")
        # cprint(f"any: {torch.any(self.fingertip_pos[..., 2] <= self.cfg.hit_threshold, dim=-1)}", "red")

        # cprint(f"torch.any(fingertip_pos_z <= hit_threshold, dim=-1): {torch.any(self.fingertip_pos[..., 2] <= self.cfg.hit_threshold, dim=-1)}", "red")

        # if torch.any(torch.any(self.fingertip_pos[..., 2] <= self.cfg.hit_threshold, dim=-1)):
        #     cprint(f"hit the ground! {torch.where(torch.any(self.fingertip_pos[..., 2] <= self.cfg.hit_threshold, dim=-1))}", "red")
        #     cprint(f"self.reset_goal_buf,: {self.reset_goal_buf}", "cyan")
        #     time.sleep(5)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        hit_ground = torch.any(self.fingertip_pos[..., 2] <= self.cfg.hit_threshold, dim=-1)
