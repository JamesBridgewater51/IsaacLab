...
from isaaclab_tasks.direct.shadow_hand_inhand_push.shadow_inhand_push_cfg import ShadowHandDirectInHandPushEnvCfg, ShadowInhandPushWristFixedEnvCfg

from termcolor import cprint


class ShadowInhandPushEnv(DirectRLEnv):
    cfg: ShadowHandDirectInHandPushEnvCfg | ShadowInhandPushWristFixedEnvCfg

    def __init__(self, cfg: ShadowHandDirectInHandPushEnvCfg | ShadowInhandPushWristFixedEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # self.box_x_min = self.box_pos[:, 0] - self.cfg.x_range
        # self.box_x_max = self.box_pos[:, 0] + self.cfg.x_range
        # self.box_y_min = self.box_pos[:, 1] - self.cfg.y_range
        # self.box_y_max = self.box_pos[:, 1] + self.cfg.y_range
        # self.box_z_min = self.box_pos[:, 2] - self.cfg.z_range
        # self.box_z_max = self.box_pos[:, 2] + self.cfg.z_range
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        # for debug
        # self.actions[:, 1] = -1.0
        # cprint(f"actions: {self.actions}", "cyan")
    ...
    def _reset_target_pose(self, env_ids):
        ...
        # # some ajustments
        # new_pos[:, 2] = new_pos[:, 2].clip(self.cfg.hand_center[2], torch.inf)   # z轴不低于手心

        # cprint(f"new_pos: {new_pos}", "magenta")
        # # 当y轴的位置位于(-box_pos, 0)之间的时候，z轴高度减半
        # back_indices = torch.where((new_pos[:, 1] > self.cfg.box_pos[1]) & (new_pos[:, 1] < 0))
        # new_pos[back_indices, 2] = (new_pos[back_indices, 2] - self.cfg.hand_height) / 2 + self.cfg.hand_height

        # cprint(f"back_indices: {back_indices}", "cyan")
        # cprint(f"new_pos: {new_pos}", "cyan")
