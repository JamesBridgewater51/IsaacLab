...
from isaaclab_tasks.direct.shadow_hand_inhand_push.shadow_inhand_push_cfg import TakeBallsDownEnvCfg
...
ENV TO TAKE A PC OF OUR TARGET GREEN BALL. 

'''

take_cnt = 0

class TakeBallsDownEnv(DirectRLEnv):
    cfg: TakeBallsDownEnvCfg

    def __init__(self, cfg: TakeBallsDownEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        #### test in 10.27 ###########
        self.goal_pos[:, :] = torch.tensor([0.0, -0.35, 0.5 + cfg.ball_radius], device=self.device)
        # self.goal_pos[:, :] = self.goal_pos + self.scene.env_origins
        ...
        self.reset_cnt = 0
        cprint(f"cfg.object_name: {cfg.object_name}", "red")
    ...
    def _setup_scene(self):
        ...
        # add two cameras at (0.1, 0.1, 0.1) and (-0.1, -0.1, -0.1) respectively, and they look at (0, 0, 0), using rotation quaternions to do so   
        self.camera_00 = Camera(self.cfg.camera_config_00)
        self.camera_01 = Camera(self.cfg.camera_config_01)
        self.camera_02 = Camera(self.cfg.camera_config_02)
        self.camera_03 = Camera(self.cfg.camera_config_03)

        self.camera_04 = Camera(self.cfg.camera_config_04)
        ...
        self.scene.sensors["camera_02"] = self.camera_02
        self.scene.sensors["camera_03"] = self.camera_03

        self.scene.sensors["camera_04"] = self.camera_04
        # add lights
        light_cfg = sim_utils.DomeLightCfg(intensity=450.0, color=(0.75, 0.75, 0.75))
    ...
    def _get_observations(self) -> dict:
        ...
        # generate pointcloud from depth and RGB image, save pcd to a numpy file and exit the program
        ...
        # get rgba, depth, intrinsic_matrices, pos_w, quat_w_ros from camera sensor
            obs_collect[f"rgba_img_0{cam_id}"] = self.scene[f"camera_0{cam_id}"].data.output["rgb"]
            obs_collect[f"depth_img_0{cam_id}"] = self.scene[f"camera_0{cam_id}"].data.output["distance_to_image_plane"]
            obs_collect[f"intrinsic_matrices_0{cam_id}"] = self.scene[f"camera_0{cam_id}"].data.intrinsic_matrices
            obs_collect[f"pos_w_0{cam_id}"] = self.scene[f"camera_0{cam_id}"].data.pos_w
            obs_collect[f"quat_w_ros_0{cam_id}"] = self.scene[f"camera_0{cam_id}"].data.quat_w_ros

        points_all = []
        colors_all = []

        for cam_id in range(num_cameras):
            rgba_all = obs_collect.get(f"rgba_img_0{cam_id}", None)
            depth_all = obs_collect.get(f"depth_img_0{cam_id}", None)
            intrinsic_matrices_all = obs_collect.get(f"intrinsic_matrices_0{cam_id}", None)
            pos_w_all = obs_collect.get(f"pos_w_0{cam_id}", None)
            quat_w_ros_all = obs_collect.get(f"quat_w_ros_0{cam_id}", None)

            rgba = rgba_all[0]
            depth = depth_all[0]
            intrinsic_matrix = intrinsic_matrices_all[0]
            pos_w = pos_w_all[0]
            quat_w_ros = quat_w_ros_all[0]
        ...
        # convert it into .ply file to debug
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(points_all.cpu().numpy())
        pcd.colors = o3d.utility.Vector3dVector(colors_all.cpu().numpy() / 255.0)
        # farthest sampling using open3d
        pcd_sampled = o3d.geometry.PointCloud.farthest_point_down_sample(pcd, 50000)

        points_sampled_all = pcd.points
        colors_sampled_all = pcd.colors

        points_sampled = torch.tensor(np.array(points_sampled_all), device=self.device)
        colors_sampled = torch.tensor(np.array(colors_sampled_all), device=self.device)
        # draw pcd
        ...
        take_cnt += 1
        if take_cnt == 2:
        # o3d.io.write_point_cloud("assets/point_clouds/object_colored.ply", pcd_sampled)
            o3d.visualization.draw_geometries([pcd_sampled])
            o3d.io.write_point_cloud(self.cfg.ply_path, pcd_sampled)
            point_color_sampled = torch.cat([points_sampled, colors_sampled], dim=-1)
            cprint(f"self.object_pos: {self.object_pos}", "magenta")
            cprint(f"point_color: {point_color_sampled.shape}", "cyan")
            # save point cloud
            # torch.save(point_color_sampled, "assets/point_clouds/object_colored.pt")
            torch.save(point_color_sampled, self.cfg.pt_path)
            # cprint(f"save pointcloud to: {os.path.abspath('assets/point_clouds/object_colored.pt')}", "cyan")
            cprint(f"save pointcloud to: {self.cfg.pt_path}", "cyan")
            cprint(f"points_sampled.range along 3 axis respectively", "green")
            cprint(f"x: {points_sampled[:, 0].min()}, {points_sampled[:, 0].max()}")
            cprint(f"y: {points_sampled[:, 1].min()}, {points_sampled[:, 1].max()}")
            cprint(f"z: {points_sampled[:, 2].min()}, {points_sampled[:, 2].max()}")
            cprint(f"colors_sampled.range along 3 axis respectively", "green")
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # 要改成和get_rewards里面一样的，不然的话会出现reset了 但是没有惩罚
        goal_dist = torch.norm(self.object_pos - self.goal_pos, p=2, dim=-1)
        out_of_reach = goal_dist >= self.cfg.fall_dist
        # cprint(f"goal_dist: {goal_dist}", "cyan")
        # cprint(f"out_of_reach: {out_of_reach}", "cyan")
    ...
    def _reset_target_pose(self, env_ids):
        ...
        new_pos[:, 2] = new_pos[:, 2].clip(self.cfg.hand_center[2], torch.inf)

        self.goal_pos[env_ids] = new_pos

        goal_pos = self.goal_pos + self.scene.env_origins
        # cprint(f"goal_pos: {goal_pos}")
        # goal_pos = self.goal_pos + self.scene.env_origins
...
def compute_rewards(
    ...
    if dof_vel_thresh > 0: 
        goal_resets = torch.where(
        torch.abs(goal_dist) <= success_tolerance, torch.ones_like(reset_goal_buf) & (object_linvel_norm <= obj_lin_vel_thresh) & (object_angvel_norm <= obj_ang_vel_thresh) & (dof_vel_norm <= dof_vel_thresh) , reset_goal_buf
        )
    else:
        goal_resets = torch.where(
            torch.abs(goal_dist) <= success_tolerance, torch.ones_like(reset_goal_buf) & (object_linvel_norm <= obj_lin_vel_thresh) & (object_angvel_norm <= obj_ang_vel_thresh), reset_goal_buf
            )
